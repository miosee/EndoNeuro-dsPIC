...
void AcquisitionFiles::start(LockingQueue* queue) {
    ...
    lockPrintln("AcquisitionFiles finished, " + to_string(count) + " sets of files created.");
}


void AcquisitionFiles::writeToFile() { //Toute cette fonction répétée pour chaque sample
    ...
    filename = trgt + "Rpi56-" + to_string(curTime) + ".csv";
    ofstream ch56File(filename);
    filename = trgt + "Rpi78-" + to_string(curTime) + ".csv";
    ofstream ch78File(filename);
    i = 0;
    while (i < FILE_SIZE) {
      ...
      ch34File << to_string(buffer[i++]) << endl; //ch12File = fichier du VENG? Car 20 samples dedans. Par contre comment on différencie les 2 channels dans le ch12File?
	  ch56File << to_string(buffer[i++]) << ",";
      ch56File << to_string(buffer[i++]) << endl;
      ch78File << to_string(buffer[i++]) << ",";
      ch78File << to_string(buffer[i++]) << endl;
